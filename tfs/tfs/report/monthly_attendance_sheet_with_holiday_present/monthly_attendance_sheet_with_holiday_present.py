...
def get_columns(filters: Filters) -> List[Dict]:
	...
	if filters.summarized_view:
		columns.extend(
			[
				...
				{"label": _("Total Leaves"), "fieldname": "total_leaves", "fieldtype": "Float", "width": 90},
    ...
    {"label": _("WOP"), "fieldname": "WOP", "fieldtype": "Float", "width": 120},
    {"label": _("HOP"), "fieldname": "HOP", "fieldtype": "Float", "width": 120},
			]
		)
...
def get_attendance_status_for_summarized_view(
	...
	"""
	summary, attendance_days,total_holidays_present, total_weeklyoff_present = get_attendance_summary_and_days(employee, filters)
	if not any(summary.values()):
		return {}

	total_days = get_total_days_in_month(filters)
	total_holidays = total_unmarked_days = 0
	HOP = total_holidays_present
	WOP = total_weeklyoff_present
	for day in range(1, total_days + 1):
...
def get_attendance_summary_and_days(employee: str, filters: Filters) -> Tuple[Dict, List]:
	Attendance = frappe.qb.DocType("Attendance")
	Holiday = frappe.qb.DocType("Holiday")
    # Fetching the employee object to get the holiday list
	employee_doc = frappe.get_doc("Employee", employee)
	employee_holiday_list = employee_doc.holiday_list
    # Fetching holiday dates for the given holiday list
	holiday_dates = (
        frappe.qb.from_(Holiday)
        .select("holiday_date")
        .where(
            (Holiday.parent == employee_holiday_list)
	...
	weekly_holiday_dates = (
        ...
        .where(
            (Holiday.parent == employee_holiday_list)
            & (Holiday.weekly_off == 1)
            & (Extract("month", Holiday.holiday_date) == filters.month)
            & (Extract("year", Holiday.holiday_date) == filters.year)
        )
    ).run(pluck=True)
    # Fetching attendance for the given employee and filters
	attendance_data = (
        frappe.qb.from_(Attendance)
	...
	total_holidays_present = sum(1 for att in attendance_data if att["attendance_date"] in holiday_dates)
	total_weeklyoff_present = sum(1 for att in attendance_data if att["attendance_date"] in weekly_holiday_dates)
	...
	return summary[0], days, total_holidays_present, total_weeklyoff_present
