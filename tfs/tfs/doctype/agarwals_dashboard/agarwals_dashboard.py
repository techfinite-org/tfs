...
def settled_amount():
    region = get_region()
    doc = frappe.get_doc("Agarwals Dashboard", region)
    return  (doc.settled_amount)
@frappe.whitelist()
def tds_amount():
    region = get_region()
    doc = frappe.get_doc("Agarwals Dashboard", region)
    return  (doc.tds_amount)
@frappe.whitelist()
def open_receipt():
    region = get_region()
    doc = frappe.get_doc("Agarwals Dashboard", region)
    return  (doc.open_receipt)
@frappe.whitelist()
def open_bills():
    region = get_region()
    doc = frappe.get_doc("Agarwals Dashboard", region)
    return  (doc.open_bills)
...
def value_updater():
    ...
    for every_region in region_list:
        if every_region == "All":
            ...
            collection = frappe.db.sql("""select sum(deposit) as deposit from `tabBank Transaction` where coalesce(`tabBank Transaction`.`creation`, '0001-01-01 00:00:00.000000') < '2024-02-04 01:41:12.310417'""")
            ...
            collection = frappe.db.sql("""select sum(deposit) as deposit from `tabBank Transaction` where custom_region = %s AND coalesce(`tabBank Transaction`.`creation`, '0001-01-01 00:00:00.000000') < '2024-02-04 01:41:12.310417' """, every_region)
